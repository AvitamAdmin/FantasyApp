  ...
  const fetchPlayerRole = async (playerRoleId, token) => {
    try {
      ...
      const filteredPlayerRoles = response.data.playerRoleDtoList.filter(
        item =>
          item.playerRole === 'Batsman' &&
          Array.isArray(playerRoleId) &&
          playerRoleId.includes(item.recordId),
        // setBatsManId(item.recordId)
      );

      // batsmanId

      const batsman = response.data.playerRoleDtoList.find(
        item => item.playerRole === 'Batsman',
      );
      if (batsman) {
        setId(batsman.recordId);
      }

      // Extract record IDs
      const BatsmanRecordIds = filteredPlayerRoles.map(item => item.recordId);

      setBatsmanId(BatsmanRecordIds);

      console.log(BatsmanRecordIds, 'BatsmanRecordIds from playerRoleList');
    } catch (error) {
          ...
          <View style={{flex: 1, alignItems: 'center', gap: 5}}>
            ...
            <View style={{flex: 1, alignItems: 'center', gap: 10}}>
              {Player.map((item, index) => {
                return item.playerRoleId == BatsmanId ? (
                  <Pressable
                    onPress={() => {

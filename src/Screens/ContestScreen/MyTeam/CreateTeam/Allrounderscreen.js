...
const Allrounderscreen = () => {
  ...
  const [AllrounderId, setAllrounderId] = useState([]);
  ...
  const fetchPlayerRole = async (playerRoleId, token) => {
    try {
      ...
      const filteredPlayerRoles = response.data.playerRoleDtoList.filter(
        item =>
          item.playerRole === 'All Rounders' &&
          Array.isArray(playerRoleId) &&
          playerRoleId.includes(item.recordId),
        // setAllrounderId(item.recordId)
      );

      //allrounderId
      const allrounder = response.data.playerRoleDtoList.find(
        item => item.playerRole === 'All Rounders',
      );
      if (allrounder) {
        setId(allrounder.recordId);
      }
      // Extract record IDs
      const AllrounderRecordIds = filteredPlayerRoles.map(
        item => item.recordId,
      );

      setAllrounderId(AllrounderRecordIds);

      console.log(
        AllrounderRecordIds,
        'AllrounderRecordIds from playerRoleList',
      );
    } catch (error) {
          ...
          <View style={{flex: 1, alignItems: 'center', gap: 5}}>
            ...
            <View style={{flex: 1, alignItems: 'center', gap: 10}}>
              {Player.map((item, index) => {
                return item.playerRoleId == AllrounderId ? (
                  <Pressable
                    onPress={() => {
